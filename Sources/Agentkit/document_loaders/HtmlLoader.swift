...
//  HtmlLoader
//
//  Created by Simon Weniger on 2024/7/19.
//
import SwiftSoup
import Foundation
...
public class HtmlLoader: BaseLoader {
    ...
    public override func _load() async throws -> [Document] {
        do {
            ...
            throw AgentkitError.LoaderError("Parse html fail with \(message)")
        } catch {
            print("Get body error \(error)")
            throw AgentkitError.LoaderError("Parse html fail with \(error)")
        }
    }
